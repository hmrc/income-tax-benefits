...
import models.{AddStateBenefitRequestModel, CreateUpdateOverrideStateBenefit, StateBenefitTypes, UpdateStateBenefitModel}
...
class StateBenefitsController @Inject() (
  ...
  def getStateBenefits(nino: String, taxYear: Int, benefitId: Option[String]): Action[AnyContent] = auth.async {
    implicit user =>
  ...
  def deleteStateBenefit(nino: String, taxYear: Int, benefitId: String): Action[AnyContent] = auth.async {
    implicit user =>
  ...
  def deleteOverrideStateBenefit(nino: String, taxYear: Int, benefitId: String): Action[AnyContent] = auth.async {
    implicit user =>
  ...
  def createUpdateOverrideStateBenefit(nino: String, taxYear: Int, benefitId: String): Action[AnyContent] = auth.async {
    implicit user =>
      user.body.asJson.map(_.validate[CreateUpdateOverrideStateBenefit]) match {
        case Some(JsSuccess(model, _)) =>
          service.createUpdateStateBenefitOverride(nino, taxYear, benefitId, model).map {
            case Right(_)         => NoContent
            case Left(errorModel) => Status(errorModel.status)(errorModel.toJson)
          }
  ...
  def ignoreStateBenefit(nino: String, taxYear: Int, benefitId: String, ignoreBenefit: Boolean): Action[AnyContent] =
  ...
  def unignoreStateBenefit(nino: String, taxYear: Int, benefitId: String): Action[AnyContent] = auth.async {
    implicit user =>
    ...
    user.body.asJson.map(_.validate[AddStateBenefitRequestModel]) match {
      case Some(JsSuccess(model, _)) =>
        if (StateBenefitTypes(model.benefitType).isDefined) {
          ...
          logger.warn(
            "[StateBenefitsController][addStateBenefit] The AddStateBenefit request body has an invalid benefit type"
          )
  ...
  def updateStateBenefit(nino: String, taxYear: Int, benefitId: String): Action[AnyContent] = auth.async {
    implicit user =>
